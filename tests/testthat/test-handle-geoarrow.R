...
test_that("wk_handle() works for geoarrow.wkt", {
  ...
  arr <- geoarrow_create_narrow_from_buffers(src, schema = geoarrow_schema_wkt())
...
test_that("wk_handle() works for geoarrow.wkt stream", {
  ...
  stream_fun <- function() {
    ...
    stream_i <<- stream_i + 1
    geoarrow_create_narrow_from_buffers(src[stream_i], schema = schema, strict = TRUE)
  }
...
test_that("wk_handle() works for geoarrow.wkb", {
  src <- wk::wkt(c("POINT (0 1)", "LINESTRING (1 1, 2 2)", NA))
  arr <- geoarrow_create_narrow_from_buffers(src, schema = geoarrow_schema_wkb())
  expect_identical(wk::wk_handle(arr, wk::wkt_writer()), src)
})
