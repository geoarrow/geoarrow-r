...
test_that("wk_crs() and wk_is_geodesic() works for arrays", {
  arr <- geoarrow_create_narrow_from_buffers(wk::wkt("POINT (1 2)", crs = "OGC:CRS84"))
  expect_identical(wk_crs(arr), "OGC:CRS84")
  expect_false(wk_is_geodesic(arr))

  arr <- geoarrow_create_narrow_from_buffers(wk::wkt("LINESTRING (1 2, 3 4)", geodesic = TRUE))
  expect_true(wk::wk_is_geodesic(arr))

  arr <- geoarrow_create_narrow_from_buffers(wk::wkt("POINT (1 2)"), schema = geoarrow_schema_wkt())
  expect_null(wk_crs(arr))

  arr <- geoarrow_create_narrow_from_buffers(
  ...
  arr <- geoarrow_create_narrow_from_buffers(wk::wkt("LINESTRING (1 2, 3 4)"))
  expect_null(wk_crs(arr))
})


test_that("wk_set_crs() and wk_set_geodesic() work for arrays", {
  arr <- geoarrow_create_narrow_from_buffers(wk::wkt("POINT (1 2)"))
  ...
  arr <- geoarrow_create_narrow_from_buffers(wk::wkt("MULTILINESTRING ((1 2, 3 4))"))
  wk::wk_is_geodesic(arr) <- TRUE
  expect_true(wk::wk_is_geodesic(arr))
  wk::wk_is_geodesic(arr) <- FALSE
  expect_false(wk::wk_is_geodesic(arr))
...
test_that("wk_set_crs() and wk_set_geodesic() work for vctrs", {
  ...
  vctr <- geoarrow_create(wk::wkt("MULTILINESTRING ((1 2, 3 4))"))
  wk::wk_is_geodesic(vctr) <- TRUE
  expect_true(wk::wk_is_geodesic(vctr))
  wk::wk_is_geodesic(vctr) <- FALSE
  expect_false(wk::wk_is_geodesic(vctr))
