...
In addition to nested list types, also define "flat" types that can be streamed without waiting for a feature boundary. This is helpful for algorithms that don't need all the coordinates in memory at once or for streaming very large geometries over a network connection.
