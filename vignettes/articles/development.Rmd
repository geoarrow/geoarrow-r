...
### Using the GeoArrowHandler

The `GeoArrowHandler` is a class with methods corresponding to events that occur when looping over coordinates from top to bottom. This style of iteration was inspired by GeoRust's [geozero](https://github.com/georust/geozero) and SAX-style XML parsers. Writing operations as handlers is a good fit for some operations but can be challenging for others. In R, it powers the `wk::wk_handle()` operation which in turn powers many other operations via wk handlers elsewhere. It is particularly useful for operations that iterate forward through all coordinates.

For example, the following subclass of `GeoArrowHandler` calculates the bounding box of any (arrow array of) geometry it is handed.

```{cpp11}
#include <cpp11.hpp>
#include "narrow.h"
#include "../../src/internal/geoarrow.hpp"

using namespace cpp11;
using namespace geoarrow;

class BboxHandler: public GeoArrowHandler {
public:
  BboxHandler()
  ...
  GeoArrowHandler::Result coord(const double* coord) {
...
doubles geoarrow_bbox(sexp schema_xptr, sexp array_data_xptr) {
  ...
  std::unique_ptr<GeoArrowArrayView> view(create_view(schema));
...
The geoarrow package uses the `GeoArrowMeta` class to validate and parse both schema and array objects. It is a wrapper around a `struct ArrowSchema` that walks its input and extracts pieces in a reasonable format to build on. In most cases you shouldn't need to extract metadata in compiled code; however, the `GeoArrowMeta` class is the easiest way to do so if you are in this position.
...
void geoarrow_metadata_dump(sexp schema_xptr) {
  ...
  case GeoArrowMeta::Extension::Multi:
    Rprintf("Extension: 'geoarrow.multi'\n");
    break;
...
SEXP c_point_struct_print(SEXP array_data_xptr) {
  ...
  for (int64_t point_id = 0; point_id < array_data->length; point_id++) {
    Rprintf("point[%d] (%g %g)\n", point_id, x[point_id], y[point_id]);
  }
