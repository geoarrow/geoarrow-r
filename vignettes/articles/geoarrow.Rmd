...
places_folder <- system.file("denmark_osm/osm_places", package = "geoarrow")
...
(When some kind of index for dataset files exists, talk about how to use it)

## Reading a data set hosted remotely

(Wait until we have some properly formatted files on a public S3 bucket)

## Writing a data set

(Wait until a `write_geoarrow_dataset()` exists)

## Using `geoarrow_collect()`

Until this point, we have used `geoarrow_collect_sf()`. This is probably what you want, since the fantastic [sf](https://r-spatial.github.io/sf/) package is a feature-complete GIS designed to work well with GIS data in data.frame form. If you want to customize how geometry columns are converted into R objects, you can use `geoarrow_collect()` with a `handler`. For example, if you want to convert geometry to a `geos::geos_geometry()` directly without pivoting through sf, you can use `handler = geos::geos_geometry_writer`.

```{r}
places %>% 
  head() %>% 
  geoarrow_collect(handler = geos::geos_geometry_writer)
```
